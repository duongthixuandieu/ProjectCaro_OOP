...
void _Game::Undo(int isBOT)
{
	...
	if (_undo.empty() == 0)
	{
		if (isBOT != 0) //Nếu có máy đánh thì xóa 2 nước đi gần nhất.
						//2 người đánh xóa 1 nước đi gần nhất
		{
			p = _undo.top();
			_undo.pop();
			for (int i = 0; i < _b->getSize(); i++) {
				for (int j = 0; j < _b->getSize(); j++) {
					if (_b->getXAt(i, j) == p.getX() &&
						_b->getYAt(i, j) == p.getY())
					{
						_b->setCheckAt(i, j, 0);
						break;
					}
...
int _Game::processFinish()
{
	...
	case -1: case 1: case 0:
		PlaySound("win.wav", NULL, SND_FILENAME | SND_LOOP | SND_ASYNC);
		HieuUng(pWhoWin);
	case 2:
...
void _Game::ShowInfo()
{
	...
	_Common::textcolor(13);
	cout << _winY;
	_Common::gotoXY(_b->getLeft() + 4 * _b->getSize() + 5, _b->getTop() + 20);
	_Common::textcolor(11);
	cout << "Press 'W', 'S', 'A', 'D'";
	_Common::gotoXY(_b->getLeft() + 4 * _b->getSize() + 5, _b->getTop() + 21);
	cout << "to move Up Down Left Right";
	_Common::gotoXY(_b->getLeft() + 4 * _b->getSize() + 5, _b->getTop() + 22);
	cout << "Press 'L' to Save Game  ";
	_Common::gotoXY(_b->getLeft() + 4 * _b->getSize() + 5, _b->getTop() + 23);
	cout << "Press 'Esc' to Quit Game";
	_Common::gotoXY(_b->getLeft() + 4 * _b->getSize() + 5, _b->getTop() + 24);
	...
	while (isContinue())
	{
		...
		else
		{
			...
			case 32: //undo
				...
				break;
			case 13:
				//Đánh dấu bàn cờ, sau đó kiểm tra 
				//và xử lý thắng/thua/hòa/tiếp tục
...
bool _Game::loadGame(int &isBOT)
{
	...
	for (int i = 0; i < _b->getSize(); i++)
	{
		...
		{
			infile >> temp;
			_b->_pArr[i][j].setX(4 * j + left + 2); 
			_b->_pArr[i][j].setY(2 * i + top + 1); 
			_b->setCheckAt(i, j, temp);
		}
	}
	infile >> temp;_turn = temp;
	infile >> temp;_x = temp;
	infile >> temp;_y = temp;
	infile >> temp;_winX = temp;
	infile >> temp;_winY = temp;
	infile >> temp;isBOT = temp;
	return true;
}
void _Game::HieuUng(int n)
{
	...
	int flag2 = 0; x = 1; y = 1; tt = RIGHT;
	while (flag2 == 0)
	{  _Common::gotoXY(x, y); _Common::textcolor(8 + rand() % 8);
		if (tt == RIGHT || tt == LEFT) cout << Chuoi[0];
		else cout << Chuoi[1];
		// xử lý chạm biên
...
{
	_Common::gotoXY(x, y);
	cout << "Y88b   d88P      888       888 d8b 888      888  " ;
	_Common::gotoXY(x, y+1);
	cout << " Y88b d88P       888   o   888 Y8P 8888     888   " ;
	_Common::gotoXY(x, y+2);
	cout << "  Y88o88P        888  d8b  888     88888    888 " ;
	_Common::gotoXY(x, y+3);
	cout << "   Y888P         888 d888b 888 888 888 88   888" ;
	_Common::gotoXY(x, y+4);
	cout << "   d888b         888d88888b888 888 88  888  888" ;
	_Common::gotoXY(x, y+5);
	cout << "  d88888b        88888P Y88888 888 888   888888 " ;
	_Common::gotoXY(x, y+6);
	cout << " d88P Y88b       8888P   Y8888 888 888    88888 " ;
