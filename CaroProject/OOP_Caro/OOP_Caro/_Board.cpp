...
int _Board::testPoint(int i, int j){
	...
	if (_pArr[i][j].getCheck() == -1)
	{
		...
		for (int k = 1; k <= 4; k++)
		{	//đường ngang
			if (j + k < _size && _pArr[i][j + k].getCheck() == -1)
				demngangX++;
			if (demngangX == 5) 
				return -1;
			//đường dọc
			if (i + k < _size && _pArr[i + k][j].getCheck() == -1) 
				demdocX++;
			if (demdocX == 5) 
				return -1;
			//đường chéo chính
			if (i + k < _size && j + k < _size && _pArr[i + k][j + k].getCheck() == -1) 
				demcheochinhX++;
			if (demcheochinhX == 5) return -1;
			//đường chéo phụ
			if (i + k < _size && j - k >= 0 && _pArr[i + k][j - k].getCheck() == -1) 
				demcheophuX++;
			if (demcheophuX == 5)
				return -1;
		}
